...
Available Products:
Credit Cards:
{pd.read_csv('data/Credit_Card_Details.csv').to_json(orient='records', indent=2)}

Loans:
{pd.read_csv('data/Loan_Details.csv').to_json(orient='records', indent=2)}
...
Important:
1. Only recommend products from the available_products list
...
Important:
1. Only recommend credit cards from the available list
2. Include specific user behavior analysis in the reason and user_behavior_match fields
